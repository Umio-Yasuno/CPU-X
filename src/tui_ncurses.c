	...
	switch(opts->selected_page)
	{
		...
		case NO_GRAPHICS:
			...
			for(i = 0; i < data->gpu_count * GPUFIELDS; i += GPUFIELDS)
			{
				...
				mvwprintw2c(win, line, info.tm, "%13s: %s", data->tab_graphics[NAME][GPU1POWERAVG  + i], data->tab_graphics[VALUE][GPU1POWERAVG      + i]);
				mvwprintw2c(win, line, info.tm, "%18s: %s", data->tab_graphics[NAME][GPU1VOLTAGE  + i], data->tab_graphics[VALUE][GPU1VOLTAGE      + i]);
				line += LINE_3 - 1;
			}
			break;
		case NO_BENCH:
